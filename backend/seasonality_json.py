...
class SeasonalityAnalysisGenerator:
    ...
    def load_and_prepare_data(self):
        ...
        for col in self.df.columns:
            if any(
                possible_name.lower() in col.lower()
                for possible_name in ["announced", "date"]
            ):
                date_column = col
        ...
        if date_column is None:
            raise ValueError(
                "Could not find a date column. Available columns: "
                + str(list(self.df.columns))
            )

        # Analyze the date column BEFORE conversion
        print(f"\n=== Date Column Analysis ===")
        print(f"Data type: {self.df[date_column].dtype}")

        # Check for empty values
        empty_count = self.df[date_column].isna().sum()
        if self.df[date_column].dtype == "object":
            empty_string_count = (self.df[date_column] == "").sum()
            empty_string_count += (self.df[date_column].str.strip() == "").sum()
        else:
            empty_string_count = 0

        print(f"Empty/NaN cells: {empty_count}")
        print(f"Empty string cells: {empty_string_count}")

        # Show sample of date values
        print(f"\nSample date values (first 5):")
        print(self.df[date_column].head())

        # Convert to datetime with explicit handling
        self.df["Announced Date"] = pd.to_datetime(
            self.df[date_column], format="%Y-%m-%d %H:%M:%S", errors="coerce"
        )

        # Count how many failed to parse
        nat_count = self.df["Announced Date"].isna().sum()
        print(f"\nFailed to parse: {nat_count} dates")

        # Check for any 1970 dates before filtering
        if not self.df["Announced Date"].isna().all():
            years_found = self.df["Announced Date"].dt.year.value_counts().sort_index()
            print(f"\nYears found after parsing:")
            print(years_found)

            if 1970 in self.df["Announced Date"].dt.year.values:
                count_1970 = (self.df["Announced Date"].dt.year == 1970).sum()
                print(
                    f"\n⚠️ WARNING: Found {count_1970} dates from 1970 - these will be removed"
                )

        # Remove rows with invalid dates
        initial_count = len(self.df)

        # First remove NaT values
        self.df = self.df[self.df["Announced Date"].notna()]

        # Then remove any 1970 dates (parsing errors)
        self.df = self.df[self.df["Announced Date"].dt.year != 1970]

        # Also remove any unrealistic dates (optional - adjust range as needed)
        self.df = self.df[
            (self.df["Announced Date"].dt.year >= 2000)
            & (self.df["Announced Date"].dt.year <= 2030)
        ]
        ...
        print(f"  - Invalid/empty dates: {nat_count}")
        print(f"  - 1970 dates: {count_1970 if '1970' in locals() else 0}")
        print(
            f"  - Outside 2000-2030 range: {initial_count - final_count - nat_count - (count_1970 if '1970' in locals() else 0)}"
        )

        # Verify final date range
        print(
            f"\nFinal date range: {self.df['Announced Date'].min()} to {self.df['Announced Date'].max()}"
        )
        print(
            f"Years in final dataset: {sorted(self.df['Announced Date'].dt.year.unique())}"
        )
        ...
        print(
            f"\n✅ Successfully loaded {final_count} valid records from {self.df['Year'].min()} to {self.df['Year'].max()}"
        )
        ...
        self.results["summary"] = {
            ...
            "peak_periods": {
                "month": {
                    ...
                    "percentage": round(
                        float(monthly_counts.iloc[0] / len(self.df) * 100), 1
                    ),
                },
                "quarter": {
                    ...
                    "percentage": round(
                        float(quarterly_counts.iloc[0] / len(self.df) * 100), 1
                    ),
                },
                "season": {
                    ...
                    "percentage": round(
                        float(seasonal_counts.iloc[0] / len(self.df) * 100), 1
                    ),
                },
                "day": {
                    ...
                    "percentage": round(
                        float(dow_counts.iloc[0] / len(self.df) * 100), 1
                    ),
                },
                "year": {
    ...
    def calculate_distributions(self):
        ...
        monthly_counts = (
            self.df.groupby("Month_Name").size().reindex(monthly_order, fill_value=0)
        )
        ...
        seasonal_counts = (
            self.df["Season"].value_counts().reindex(season_order, fill_value=0)
        )
    ...
    def calculate_trends(self):
        ...
        for year, count in yearly_counts.items():
            ...
            self.results["trends"]["yearly"].append(
                {"year": int(year), "count": int(count), "growthRate": growth_rate}
            )
            prev_count = count
        ...
        sample_dates = pd.date_range(
            start=daily_counts.index.min(), end=daily_counts.index.max(), freq="W"
        )

        self.results["trends"]["movingAverages"] = {
            "dates": [d.strftime("%Y-%m-%d") for d in sample_dates],
            "daily": [
                int(daily_counts[d]) if d in daily_counts.index else 0
                for d in sample_dates
            ],
            "ma7": [
                (
                    round(float(ma7[d]), 2)
                    if d in ma7.index and not pd.isna(ma7[d])
                    else None
                )
                for d in sample_dates
            ],
            "ma30": [
                (
                    round(float(ma30[d]), 2)
                    if d in ma30.index and not pd.isna(ma30[d])
                    else None
                )
                for d in sample_dates
            ],
    ...
    def calculate_statistical_insights(self):
        ...
        chi2_month, p_value_month = stats.chisquare(
            monthly_counts.values, expected_monthly
        )

        # Quarterly test
        quarterly_counts = self.df["Quarter"].value_counts().sort_index()
        expected_quarterly = [len(self.df) / 4] * 4
        chi2_quarter, p_value_quarter = stats.chisquare(
            quarterly_counts.values, expected_quarterly
        )

        # Seasonal test
        seasonal_counts = self.df["Season"].value_counts()
        expected_seasonal = [len(self.df) / 4] * 4
        chi2_season, p_value_season = stats.chisquare(
            seasonal_counts.values, expected_seasonal
        )
        ...
        trough_month = min(
            self.results["distributions"]["monthly"], key=lambda x: x["count"]
        )["fullMonth"]
        ...
        seasonality_strength = round(
            float((peak_count - trough_count) / (len(self.df) / 12) * 100), 1
        )
        ...
        self.results["insights"]["keyFindings"] = {
            ...
            "peakTroughRatio": round(
                float(peak_count / trough_count) if trough_count > 0 else 0, 2
            ),
            "mostActiveMonth": peak_month,
            "leastActiveMonth": trough_month,
            "yearlyGrowthTrend": (
                "Positive" if self.results["trends"].get("cagr", 0) > 0 else "Negative"
            ),
        }
                ...
                heatmap_data.append(
                    {
                        "year": int(year),
                        "month": int(month),
                        "monthName": pd.to_datetime(f"2000-{month:02d}-01").strftime(
                            "%b"
                        ),
                        "count": count,
                    }
    ...
    def run_analysis(self):
        ...
        print("=" * 60)
        print(
            f"Total announcements analyzed: {self.results['summary']['total_announcements']}"
        )
        print(
            f"Date range: {self.results['summary']['date_range']['start']} to {self.results['summary']['date_range']['end']}"
        )
        print(
            f"Peak month: {self.results['summary']['peak_periods']['month']['name']} ({self.results['summary']['peak_periods']['month']['percentage']}%)"
        )
        print(
            f"Seasonality strength: {self.results['insights']['keyFindings']['seasonalityStrength']}"
        )
